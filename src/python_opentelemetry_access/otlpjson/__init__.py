...
def iter_otlp_jsonlike_anyvalue(jsonlike: base.JSONLike) -> base.JSONLikeIter:
...
def iter_otlp_jsonlike_kv(jsonlike: base.JSONLike) -> Tuple[str, base.JSONLikeIter]:
    match jsonlike:
...
def iter_otlp_jsonlike_dict(jsonlike: base.JSONLikeList) -> base.JSONLikeDictIter:
    return base.JSONLikeDictIter((iter_otlp_jsonlike_kv(x) for x in jsonlike))


class OTLPJsonSpanEvent(base.SpanEvent):
    def __init__(self, jobj: base.JSONLikeDict):
        self.jobj = jobj

    @property
    def otlp_time_unix_nano(self) -> int:
        return int(_expect_field_type(self.jobj, "timeUnixNano", str))

    @property
    def otlp_name(self) -> str:
        return _expect_field_type(self.jobj, "name", str)

    @property
    def otlp_attributes(self) -> base.JSONLikeDictIter:
        return iter_otlp_jsonlike_dict(
            _expect_field_type(self.jobj, "attributes", list, optional=True, default={})
        )

    @property
    def otlp_dropped_attributes_count(self) -> int:
        return _expect_field_type(
...
class OTLPJsonStatus(base.Status):
    ...
    def otlp_message(self) -> Optional[str]:
        return _expect_field_type(self.jobj, "message", str, optional=True)

    ## TODO: Make sure this is the correct defaulting behaviour
    @property
    def otlp_code(self) -> int:
        return _expect_field_type(self.jobj, "code", int, optional=True, default=0)
...
def expect_dict(x: base.JSONLike) -> base.JSONLikeDict:
...
class OTLPJsonSpanLink(base.SpanLink):
    ...
    def otlp_trace_id(self) -> str:
        return _expect_field_type(self.jobj, "traceId", str)

    @property
    def otlp_span_id(self) -> str:
        return _expect_field_type(self.jobj, "spanId", str)

    @property
    def otlp_state(self) -> str:
        return _expect_field_type(self.jobj, "state", str)
    ...
    def otlp_flags(self) -> int:
        return _expect_field_type(self.jobj, "flags", int, optional=True, default=0)
...
class OTLPJsonSpan(base.Span):
    ...
    def otlp_trace_state(self) -> Optional[str]:
        return _expect_field_type(self.jobj, "traceState", str, optional=True)

    @property
    def otlp_parent_span_id(self) -> str:
        return _expect_field_type(self.jobj, "parentSpanId", str)
    ...
    def otlp_kind(self) -> OTLPJsonSpanKind:
        return OTLPJsonSpanKind(_expect_field_type(self.jobj, "kind", int))

    @property
    def otlp_start_time_unix_nano(self) -> int:
        return int(_expect_field_type(self.jobj, "startTimeUnixNano", str))

    @property
    def otlp_end_time_unix_nano(self) -> int:
        return int(_expect_field_type(self.jobj, "endTimeUnixNano", str))
    ...
    def otlp_events(self) -> Iterator[OTLPJsonSpanEvent]:
        return base.ListLikeDumpIterator(
            (
                OTLPJsonSpanEvent(expect_dict(event))
                for event in _expect_field_type(
    ...
    def otlp_links(self) -> Iterator[OTLPJsonSpanLink]:
        return base.ListLikeDumpIterator(
            (
                OTLPJsonSpanLink(expect_dict(link))
                for link in _expect_field_type(
    ...
    def otlp_status(self) -> OTLPJsonStatus:
        return OTLPJsonStatus(_expect_field_type(self.jobj, "status", dict))
...
class OTLPJsonInstrumentationScope(base.InstrumentationScope):
    ...
    def otlp_version(self) -> Optional[str]:
        return _expect_field_type(self.jobj, "version", str, optional=True)
...
class OTLPJsonScopeSpanCollection(base.ScopeSpanCollection):
    ...
    def otlp_scope(self) -> OTLPJsonInstrumentationScope:
        return OTLPJsonInstrumentationScope(
            _expect_field_type(self.jobj, "scope", dict)
        )

    @property
    def otlp_spans(self) -> Iterator[OTLPJsonSpan]:
        return (
            OTLPJsonSpan(expect_dict(span))
            for span in _expect_field_type(self.jobj, "spans", list)
        )

    @property
    def otlp_schema_url(self) -> str:
        return _expect_field_type(self.jobj, "schemaUrl", str, optional=True)
...
class OTLPJsonResourceSpanCollection(base.ResourceSpanCollection):
    ...
    def otlp_resource(self) -> OTLPJsonResource:
        return OTLPJsonResource(_expect_field_type(self.jobj, "resource", dict))

    @property
    def otlp_scope_spans(self) -> Iterator[OTLPJsonScopeSpanCollection]:
        return (
            OTLPJsonScopeSpanCollection(expect_dict(span))
            for span in _expect_field_type(self.jobj, "scopeSpans", list)
        )
...
class OTLPJsonSpanCollection(base.SpanCollection):
    ...
    def otlp_resource_spans(self) -> Iterator[OTLPJsonResourceSpanCollection]:
        return (
            OTLPJsonResourceSpanCollection(expect_dict(span))
            for span in _expect_field_type(self.jobj, "resourceSpans", list)
        )
