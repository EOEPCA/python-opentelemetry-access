...
class OpenSearchSS40Proxy(proxy.Proxy):
    def __init__(self, client: AsyncOpenSearch, page_size : int = 10):
        self.client = client
    ...
    async def query_spans_page(  # type: ignore
        ...
        if span_ids is not None:
            if len(span_ids) != 1:
                raise RuntimeError("Currently only supports selecting precisely one trace/span")
        ...
        if len(results["hits"]["hits"]) == self.page_size:
            ...
            next_page_token = results["hits"]["hits"][-1]["_source"]["startTime"].encode("ascii")
        else:
